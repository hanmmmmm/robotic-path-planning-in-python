...
class astar():
    def __init__(self, obstacle_map, sx, sy, gx, gy):
        ...
        # angle = self.calc_angle( (0,0), (0,100), (-100,100))
        # print('angle ', angle)
    ...
    def planning(self):
        ...
        while self.reached_goal == False:
            ...
            nearest_node_parent = self.nodes[nearest_node][1]
            angle = self.calc_angle(nearest_node_parent, nearest_node, (x,y))
            # print('angle ', angle)

            
            # truncate the line if too long 
            discard = False
            if angle < 180-self.angle_tole or angle > 180+self.angle_tole:
                discard = True
            ...
            if discard == False:
                ...
                obs = 0
                for cell in list(bresenham(x, y, self.gx, self.gy )):
                    if self.obstacle_map[cell[1], cell[0]] != 0:
                        obs += 1
                if obs == 0:
                    angle = self.calc_angle(nearest_node, (x,y), (self.gx, self.gy))
                    if angle > 180-self.angle_tole and angle < 180+self.angle_tole:
                        self.reached_goal = True
                        self.nodes[(self.gx, self.gy)] = [ self.nodes[(x,y)][0], (x,y) ]
                        print('Found it ')
                    # self.faster_goal = True
                    # self.reached_goal = True
                    # self.nodes[(self.gx, self.gy)] = [ self.nodes[(x,y)][0], (x,y) ]
                    # print('Found it ')

                # if self.faster_goal == False:
